...
PATHS = [
    "index.html",
    "build_in_multiple/index.html",
    "empty/index.html",
    "multiple/index.html",
    "oauth2/index.html",
    "options/index.html",
    "url/index.html",
    "sub_dir/page_in_sub_dir/index.html",
]
...
def start_mkdocs_server(tmp_path_factory):
    ...
    process = subprocess.Popen(
        [
            ...
            "http.server",
            "-d",
            os.path.join(testproject_path, "site"),
        ],
...
def test_light_dark_toggle(page_context: Page):
    url = f"{MKDOCS_URL}/index.html"

    page_context.goto(url)
    time.sleep(3)
    toggle_js = """
var formElement = document.querySelector('form.md-header__option');
var themeToggleButton = formElement.querySelector('label.md-header__button.md-icon:not([hidden])');
if (themeToggleButton) {
  themeToggleButton.click();
}"""
    page_context.evaluate(toggle_js)
    time.sleep(1)
    screenshot_path = "playwright-results/toggle-light-to-dark.png"
    page_context.screenshot(path=screenshot_path, full_page=True)

    page_context.evaluate(toggle_js)
    time.sleep(1)
    screenshot_path = "playwright-results/toggle-dark-to-light.png"
    page_context.screenshot(path=screenshot_path, full_page=True)
